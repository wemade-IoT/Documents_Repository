...
| HU01  | Ampliación de monitoreo de plantas    | Como usuario doméstico, quiero poder agregar más plantas a mi cuenta para poder monitorearlas de manera sencilla.   | Given que el usuario tiene un número de plantas limitado en su plan, <br> When decide agregar más plantas, <br> Then el sistema debe permitirle agregar plantas extra, cobrando un monto adicional por cada una.  | EP06         | 
| HU02  | Monitoreo de sensores adicionales  | Como usuario de negocio, quiero poder agregar más sensores a mi cuenta, dependiendo de la cantidad de área que debo cubrir.  | Given que el usuario tiene un número de sensores limitado en su plan, <br> When el usuario decide agregar más sensores, <br> Then el sistema debe permitirle añadir sensores extra, cobrando un monto adicional por cada sensor. | EP06         | 
| HU03  | Descuento en el primer sensor   | Como usuario doméstico, quiero recibir un descuento en el primer sensor, para que el costo inicial de monitoreo sea más accesible.    | Given que el usuario está comprando el primer sensor, <br> When lo agrega a su carrito, <br> Then el sistema debe aplicar automáticamente un descuento al primer sensor.  | EP06         | 
| HU04  | Instalación incluida en el plan  | Como usuario doméstico, quiero que la instalación de los sensores esté incluida en el precio del plan de suscripción para no tener costos adicionales.    | Given que el usuario selecciona el plan doméstico,   <br> When completa su compra, <br> Then el sistema debe mostrar que la instalación de los sensores está incluida en el precio <br> And no habrá cargos adicionales.  | EP06         | 
| HU05  | Instalación agregada al costo del servicio    | Como usuario de negocio, quiero que la instalación de los sensores esté incluida en el costo total del servicio, basado en la cantidad de sensores y área, para no tener que pagar un cargo separado por la instalación.   | Given que el usuario ha seleccionado el número de sensores <br> And el área de las plantas, <br> When el sistema calcula el precio total, <br> Then el costo de instalación debe ser agregado automáticamente al costo total del servicio, <br> And debe ser visible en la factura.  | EP06         | 
| HU01  | Integración de sensores IoT    | Como developer, quiero integrar sensores especializados que recojan datos de temperatura, humedad, y luz para el monitoreo de plantas.    | Given que los sensores están configurados, <br> When se recibe la señal de los sensores, <br> Then los datos deben ser enviados correctamente a la plataforma para su visualización.  | EP07         | 
| HU02   | API para el monitoreo de plantas    | Como developer, quiero crear una API para que los datos de los sensores se puedan consultar y actualizar en tiempo real.    | Given que la API está configurada, <br> When el usuario consulta el estado de sus plantas, <br> Then los datos deben ser enviados correctamente a la aplicación del usuario.  | EP07         | 
| HU03   | API Autenticación JWT    | Como developer, quiero implementar autenticación basada en JWT para garantizar que solo los usuarios autenticados accedan al sistema.    | Given que el usuario está en la página de inicio de sesión, <br> When ingresa sus credenciales correctamente, <br> Then el sistema debe devolver un token JWT válido.  | EP07         | 
| HU04   | Generación de reportes de consumo de recursos   | Como developer, quiero generar reportes sobre el consumo de recursos (agua, luz) basado en los datos recolectados.    | Given que los datos de consumo están disponibles,<br> When el usuario solicita un reporte de recursos, <br> Then el sistema debe mostrar el consumo por planta o área. | EP07         | 
| HU05   | Integración de pagos    | Como developer, quiero integrar un sistema de pagos para procesar las transacciones de los usuarios de forma segura.   | Given que el sistema de pagos está integrado, <br> When el usuario introduce sus datos de pago, <br> Then el sistema debe procesar el pago de forma correcta y segura.  | EP07         | 
